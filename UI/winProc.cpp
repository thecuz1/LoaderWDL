...
LRESULT CALLBACK hkWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    // 1. Give the message to ImGui first if your menu is open/active
    // (You can use a boolean toggle to check if your menu is visible, e.g., if (show_menu))
    if (ImGui_ImplWin32_WndProcHandler(hWnd, uMsg, wParam, lParam))
    {
        return true; // Stop the game from processing this input if ImGui used it
    }

    // 2. Optional: If your menu is open, you might want to block game controls
    // (like movement or shooting) by checking ImGui's IO flags:
    /*
    ImGuiIO& io = ImGui::GetIO();
    if (show_menu && (io.WantCaptureMouse || io.WantCaptureKeyboard))
    {
        // Decide whether to drop the message or pass specific ones
    }
    */
