...
static HRESULT createDummyObjects() {
	...
	HRESULT hr = D3D11CreateDeviceAndSwapChain(
		...
		D3D11_SDK_VERSION, &scDesc, &gSwapChain, &gDevice, nullptr, &gContext);
	if (FAILED(hr)) {
		Logger::LogMessage(
			"[UI/d3d11] D3D11CreateDeviceAndSwapChain (hardware) failed: 0x%08X, trying WARP\n",
			hr);
		hr = D3D11CreateDeviceAndSwapChain(nullptr, D3D_DRIVER_TYPE_WARP, nullptr, 0, featureLevels,
										   ARRAYSIZE(featureLevels), D3D11_SDK_VERSION, &scDesc,
...
HRESULT APIENTRY hookPresentD3D11(IDXGISwapChain *pSwapChain, UINT SyncInterval, UINT Flags) {
	...
	if (!gInitialized) {
		...
		if (FAILED(gDevice->CreateRenderTargetView(backBuffer.Get(), nullptr,
												   gMainRenderTargetView.GetAddressOf()))) {
		...
		// Don't let ImGui set/hide the OS cursor - the game manages it during gameplay.
		// We draw ImGui's own cursor via io.MouseDrawCursor instead.
...
HRESULT APIENTRY hookResizeBuffersD3D11(IDXGISwapChain *pSwapChain, UINT BufferCount, UINT Width,
...
#define HOOK(name, ...)                                                                            \
	mh = MH_CreateHook(p##name##Target, reinterpret_cast<LPVOID>(hook##name##D3D11),               \
